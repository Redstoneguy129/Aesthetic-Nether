...
public class HeatscarSpider extends SpiderEntity {
    ...
    public static final ResourceLocation LOOT_TABLE = new ResourceLocation(AestheticNether.MODID, "loot/entity/heatscarspider");
    ...
    public void onDeath(DamageSource p_70645_1_) {
        if(!this.world.isRemote) {
            int num = 10 - 2 + 1;
            int amount = this.rand.nextInt(num) + 2;
            for (int i = 0; i < amount; i++) {
                double f = this.rand.nextDouble() * 2;
                double f1 = this.rand.nextDouble() * 2;
                ZombieEntity babyspider = new ZombieEntity(this.world);
