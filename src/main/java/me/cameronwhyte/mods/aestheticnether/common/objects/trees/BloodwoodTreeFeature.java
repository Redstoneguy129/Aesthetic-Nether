...
public class BloodwoodTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
    private static final BlockState LOG = ANBlocks.BLOODWOOD.getDefaultState();
    private static final BlockState LEAF = ANBlocks.BLOODLEAVES.getDefaultState();

    public BloodwoodTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configIn, boolean doBlockNotifyIn, boolean extraRandomHeightIn) {
        super(configIn, doBlockNotifyIn);
        this.setSapling((net.minecraftforge.common.IPlantable)ANBlocks.BLOODSAPLING);
    }
    ...
    public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox boundsIn) {
        ...
        if (i == -1) {
            ...
            int i1 = i - 5;
            List<BloodwoodTreeFeature.FoliageCoordinates> list = Lists.newArrayList();
            list.add(new BloodwoodTreeFeature.FoliageCoordinates(position.up(i1), l));
                    ...
                    for(int j1 = 0; j1 < k; ++j1) {
                        ...
                        if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false, boundsIn) == -1) {
                            ...
                            if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false, boundsIn) == -1) {
                                list.add(new BloodwoodTreeFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
                            }
                        }
                    }
                }
    ...
    private void makeFoliage(IWorldGenerationReader worldIn, int p_208525_2_, BlockPos pos, List<BloodwoodTreeFeature.FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> changedBlocks) {
        for(BloodwoodTreeFeature.FoliageCoordinates BloodwoodTreeFeature$foliagecoordinates : p_208525_4_) {
            if (this.trimBranches(p_208525_2_, BloodwoodTreeFeature$foliagecoordinates.getBranchBase() - pos.getY())) {
                this.foliageCluster(worldIn, BloodwoodTreeFeature$foliagecoordinates, p_208525_5_, changedBlocks);
            }
        }
    ...
    private void makeBranches(Set<BlockPos> p_208524_1_, IWorldGenerationReader p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<BloodwoodTreeFeature.FoliageCoordinates> p_208524_5_, MutableBoundingBox p_208524_6_) {
        for(BloodwoodTreeFeature.FoliageCoordinates BloodwoodTreeFeature$foliagecoordinates : p_208524_5_) {
            int i = BloodwoodTreeFeature$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(p_208524_4_.getX(), i, p_208524_4_.getZ());
            if (!blockpos.equals(BloodwoodTreeFeature$foliagecoordinates) && this.trimBranches(p_208524_3_, i - p_208524_4_.getY())) {
                this.makeLimb(p_208524_1_, p_208524_2_, blockpos, BloodwoodTreeFeature$foliagecoordinates, true, p_208524_6_);
            }
        }
